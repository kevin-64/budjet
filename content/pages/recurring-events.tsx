...
const RecurringEventsContent = () => {
  ...
  const onDelete = useCallback(() => {
    if (idToDelete) {
      db.recurringEvents.remove({ _id: idToDelete }, (err: Error | null, amount: number) => {
        db.deadlines.remove(
          { eventId: idToDelete },
          { multi: true },
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params: GridCellParams) => (
        <>
          <div>{moment.utc((params.row.periodicity as any)!.start).format('MM/DD')}</div>
        </>
      ),
    },
    {
    ...
    <>
      ...
      <div className={classes.tableContainer}>
        <DataGrid
          ...
          onRowDoubleClick={row => history.push(`edit-recurring-event?id=${row.id}`)}
        />
      </div>
      <Button href="#/edit-recurring-event" className={classes.add} component={Link}>
        Add
      </Button>
      <ConfirmDialog
        prompt="this event"
